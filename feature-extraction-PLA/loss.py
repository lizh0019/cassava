...
def batch_hard(dists, pids, margin, batch_precision_at_k=None):
    ...
    with tf.name_scope("batch_hard"):
        ...
        # max_batch = dists*tf.cast(positive_mask, tf.float32)
        # furthest_positive = tf.reduce_max(max_batch, axis=1)
        # max_index = tf.where(max_batch == furthest_positive)
        # #pdb.set_trace()
        # max_batch[max_index].assign(tf.reduce_min(max_batch))
        # furthest_positive = tf.reduce_max(max_batch, axis=1)

        # min_batch = dists + 1e5*tf.cast(same_identity_mask, tf.float32)
        # closest_negative = tf.reduce_min(min_batch, axis=1)
        # min_index = tf.where(min_batch == closest_negative)
        # min_batch[min_index].assign(tf.reduce_max(min_batch, axis=1))
        # closest_negative = tf.reduce_min(min_batch, axis=1)

        #pdb.set_trace()
	top2_positive = tf.nn.top_k(dists*tf.cast(positive_mask, tf.float32), k=2)[0][:,:2]
	if top2_positive[0] > 5.0 * top2_positive[1]: # or top2_positive[0]>0.1:
            furthest_positive = top2_positive[:,1]
	else:
	    furthest_positive = top2_positive[:,0]

        top2_negative = -1*tf.nn.top_k(-1*(dists + 1e5*tf.cast(same_identity_mask, tf.float32)), k=2)[0][:,:2]
 
	if False: #top2_negative[0] < 0.2 * top2_negative[1]: # or top2_negative[0]<0.1:
            closest_negative = top2_negative[:,1]
	else:
	    closest_negative = top2_negative[:,0]
