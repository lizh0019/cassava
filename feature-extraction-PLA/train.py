    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        if args.resume:
            ...
            if args.initial_checkpoint is not None:
                var_to_restore =[v for v in model_variables if not v.name.find('customizedlayer') >0]
                saver_nofc = tf.train.Saver(var_to_restore)
                saver_nofc.restore(sess, args.initial_checkpoint)
