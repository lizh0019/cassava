    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      with tf.variable_scope('Logits'):
        ...
        if kernel_size.is_fully_defined():
          net = slim.conv2d(net, 2048, 1, scope='customizedlayer_Conv2d_1b_1x1', activation_fn=activation_fn)  #8x8x1536 --> 8x8x2048
          net = slim.max_pool2d(net, kernel_size, padding='VALID',  scope='MaxPool_1a_8x8') #1x1x2048
          net = slim.flatten(net, scope="customizedlayer1")
          #net = slim.dropout(net, dropout_keep_prob, is_training=is_training,   scope='customizedlayer_Dropout')
          #net = slim.fully_connected(net, 2048, activation_fn=tf.nn.softmax,  scope='customizedlayer2')
          print (net.get_shape())
        else:
...
def endpoints(image, is_training):
    ...
    #    endpoints['inception_resnet_v2/block4'], [1, 2], name='pool5', keep_dims=False)

    return endpoints, 'InceptionResnetV2'
